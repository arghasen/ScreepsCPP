...
inline void basicCreep()
{
    ...
    if (!sp1.spawning() or rooms.at(sp1.room().name()).energyAvailable() >= 250) {
        auto res = sp1.spawnCreep({Screeps::WORK, Screeps::CARRY, Screeps::MOVE},
                                  "creep" + std::to_string(Screeps::Game.time()));
    ...
    for (auto &&[k, c] : Screeps::Game.creeps()) {
        ...
        if (c.store().getUsedCapacity(Screeps::RESOURCE_ENERGY) == 0) {
            ...
            c.say("harvesting");
            c.setMemory(m);
        } else if (c.store().getFreeCapacity(Screeps::RESOURCE_ENERGY) == 0) {
            m["task"] = "upgrade";
            c.say("upgrading");
            c.setMemory(m);
        }
        ...
        } else if (m["task"] == "upgrade" && c.upgradeController(*controller) == Screeps::ERR_NOT_IN_RANGE) {
            c.moveTo(*controller);
        }
    }
}
